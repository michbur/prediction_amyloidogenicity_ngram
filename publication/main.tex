...
\keywords{n-gram, amyloid, prediction}
...
AmyloGram is available as a web-server: \url{www.smorfland.uni.wroc.pl/amylogram/}. The code and results are publicly available at: \url{github.com/michbur/prediction\_amyloidogenicity\_ngram}.
...
%
% ^.
  The aim of our study is opposite to choose out of thousands of created hot spot 
...
best-performing classifier, AmyloGram, on the external data set against 
...
contactivity. We preferred the measures that were introduced after 1980 because 
they were more accurate than the older ones thanks to the technological 
advancements. The set of selected physicochemical properties was enriched by 
...
obvious that QuiPT is heuristics for an unsolved problem of a two-tailed 
...
During the learning stage, random forest classifier was trained on the binary 
n-gram data drawn from the overlapping hexamers, considering only n-grams selected by 
QuiPT (Fig.~\ref{fig:ngram_scheme} G). We grown the forest keeping the default number of tree (500) and 
the default number of variables to possibly split at in each node (the rounded down square 
root of the total number of variables). To speed up the computation, we used the 
% * <kotulska@gmail.com> 2016-06-12T11:39:46.476Z:
%
% > variables to possibly split at in each node equal to the (rounded down) square 
% > root of the total number of variables
%
% Ten fragment jest trochę niezrecznie napisany i dla mnie nie do końca zrozumiały. Jakoś go trzeba przerobić.
%
% ^ <michalburdukiewicz@gmail.com> 2016-06-12T18:50:57.536Z:
%
% przerobiony
%
% ^.
fastest implementation of random forest in \textbf{R}, ranger package~\citep{wright_ranger:_2015}.
...
The encoding yielding classifier with the best ability to correctly predict amyloidogenicity 
of peptides was chosen during the five-fold cross-validation. Since the random assignment 
of peptides to subsamples in cross-validation may result in the uneven number of hexamers in 
% * <kotulska@gmail.com> 2016-06-12T11:41:20.571Z:
%
% > to subsamples
%
% trzeba sprecyzować o jakie podpróby chodzi, teraz niezrozumiałe
%
% ^ <michalburdukiewicz@gmail.com> 2016-06-12T18:52:37.741Z:
%
% dodano "in cross-validation"
%
% ^.
...
%
% ^ <michalburdukiewicz@gmail.com> 2016-06-10T08:02:44.684Z:
%
% improved
%
% ^ <michalburdukiewicz@gmail.com> 2016-06-10T08:02:49.297Z.
...
quartile of all AUC values. 
% * <kotulska@gmail.com> 2016-06-12T14:03:36.068Z:
%
% > other
%
% all?
%
% ^.
(Fig.~\ref{fig:AUC_boxplot}). It reached the highest AUC (0.8667) 
...
probably from the pattern homogeneity of the short peptides. Nevertheless, the results 
...
efficient enough to produce classifiers able to outperformed the other published 
...
we did not tested it on \textit{reg33} data set, which is commonly used to 
...
of amyloidogenic sequences called AmyloGram. One of the strengths of the assumed approach 
...
The code and results are publicly available at: \url{github.com/michbur/prediction\_amyloidogenicity\_ngram}.
